...
👉 The pod YAML lives on the **node filesystem**, and kubelet turns it into a running pod automatically.

---

## 🧠 Why Static Pods Exist

Static pods solve Kubernetes’ **bootstrap problem**.
...
✅ Static pods allow kubelet to start **critical components independently**.
...
🟡 The API server only sees a **mirror**, not the real pod.

---

## ⚙️ kubelet Configuration (Source of Truth)
...
📌 Each file represents **one static pod**.

---

## 🧪 How to Check Static Pods in Minikube (Step-by-Step)

Follow these steps **inside Minikube**, not on your host machine.

### 🔹 Step 0: SSH into Minikube node
...
### 🔹 Step 2: Navigate to static pod directory
...
✅ These files confirm that control-plane components are running as **static pods** in Minikube.
...
* 🎛️ **kube-controller-manager** — runs controllers
...
## ❓ Why Static Pod YAML Does NOT Contain `staticPodPath`
...
* ❌ Cannot be edited
* ❌ Cannot be deleted
* ❌ Cannot be controlled by controllers
...
## 🛠️ Creating Your Own Static Pod
...
## ✏️ Modifying or Deleting Static Pods

* 📝 Edit YAML → pod restarts automatically
* 🗑️ Delete YAML → pod stops
...
* 🔄 Rolling updates
* 📈 Auto-scaling
* 🧩 High availability via controllers
...
## 🏗️ Who Creates Control-Plane Static Pods?

### Two responsibilities, two components

#### 🧾 Who creates the YAML files?

👉 **kubeadm** (or Minikube internally)
...
#### ⚙️ Who creates and runs the pods?
...
🧠 kubelet is the **runtime owner** of static pods.

---

## 🧬 Control Plane Bootstrap Timeline
...
## 🔗 kubelet Bootstrap Relationship
